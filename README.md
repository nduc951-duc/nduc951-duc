# Hi, I'm Tran Huu Duc 👋

I'm a **Software Engineering student** and aspiring **Full-stack Developer** based in Ho Chi Minh City, Vietnam.

I enjoy building practical web applications and working across both frontend and backend development. My current focus is on developing scalable full-stack applications using **React, Spring Boot, Express.js, and PostgreSQL**.

## 🛠 Tech Stack

* **Languages:** Java, JavaScript, SQL, Python
* **Frontend:** React, Vite, React Router, HTML, CSS, Bootstrap
* **Backend:** Spring Boot, Spring Security, Spring Data JPA, Hibernate, Express.js
* **Database:** PostgreSQL, Supabase, SQL Server
* **Tools:** Git, GitHub, Postman, Maven, IntelliJ IDEA, VS Code
* **Concepts:** REST APIs, JWT Authentication, Role-Based Access Control, Database Design

##  Featured Project

### ReMarket — Second-hand Marketplace Platform

A full-stack marketplace application for buying and selling second-hand products.

Key features include:

* Product listing, search and category filtering
* Favorites and user profiles
* JWT authentication and role-based authorization
* RESTful APIs for products, users, orders and administration
* PostgreSQL database with Supabase
* AI-powered marketplace support assistant

**Tech Stack:** React · Vite · Express.js · PostgreSQL · Supabase · JavaScript

##  Currently

* Improving my skills in **full-stack software development**
* Building practical projects with modern web technologies
* Learning more about **software architecture, backend development and cloud technologies**
* Looking for opportunities as a **Full-stack Developer Intern / Software Engineer Intern**

## Connect with me

* GitHub: github.com/nduc951-duc
* Email: [huuductran.dev@gmail.com](mailto:huuductran.dev@gmail.com)
